...
CLAUDE_ALLOWED_TOOLS="Write,Edit,Read,Bash(git *),Glob,Grep"  # Comma-separated list of allowed tools
...
# Permission configuration (Phase 1.3)
CLAUDE_SKIP_PERMISSIONS=false           # Bypass all permission checks (dangerous!)
CLAUDE_PERMISSION_MODE=""               # Permission mode: acceptEdits, bypassPermissions, default, delegate, dontAsk, plan
CLAUDE_CHROME=""                        # Chrome integration: "true" to enable, "false" to disable, "" for default
CLAUDE_ADD_DIRS=""                      # Additional directories to allow tool access (comma-separated)
...
build_claude_command() {
    ...
    # Add permission bypass flag (dangerous - use only in sandboxes!)
    if [[ "$CLAUDE_SKIP_PERMISSIONS" == "true" ]]; then
        CLAUDE_CMD_ARGS+=("--dangerously-skip-permissions")
        log_status "WARN" "⚠️  Running with --dangerously-skip-permissions (no permission checks!)"
    fi
...
Permission Options (Phase 1.3):
    --dangerously-skip-permissions  Bypass ALL permission checks (use only in sandboxes!)
    --skip-permissions              Alias for --dangerously-skip-permissions
    ...
    --add-dir DIRECTORY             Add directory to allowed tool access (can be used multiple times)
...
Examples:
    ...
    $0 --skip-permissions       # Skip all permission prompts (DANGEROUS!)
    $0 --permission-mode dontAsk  # Auto-approve permission prompts
    $0 --add-dir /path/to/project # Allow access to additional directory
    $0 --chrome                   # Enable Chrome integration
    $0 --no-chrome                # Disable Chrome integration
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dangerously-skip-permissions|--skip-permissions)
            ...
            echo -e "\033[1;33m⚠️  WARNING: Running with --dangerously-skip-permissions\033[0m"
            echo -e "\033[1;33m   All permission checks will be bypassed!\033[0m"
            echo -e "\033[1;33m   Only use in sandboxed environments without internet access.\033[0m"
            shift
            ;;
        --permission-mode)
            if [[ -z "$2" ]]; then
            ...
            case "$2" in
                acceptEdits|bypassPermissions|default|delegate|dontAsk|plan)
                    ...
                    ;;
                *)
                    echo "Error: Invalid permission mode '$2'"
                    echo "Valid modes: acceptEdits, bypassPermissions, default, delegate, dontAsk, plan"
                    exit 1
                    ;;
            esac
            shift 2
        ...
        --add-dir)
            ...
            if [[ -n "$CLAUDE_ADD_DIRS" ]]; then
                CLAUDE_ADD_DIRS="$CLAUDE_ADD_DIRS,$2"
            else
                CLAUDE_ADD_DIRS="$2"
            fi
            shift 2
